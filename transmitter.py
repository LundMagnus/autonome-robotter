...
duration = 1       # Tone duration in seconds

# Function to generate DTMF tone for a key
def generate_dtmf_tone(key, duration=0.5, sample_rate=44100, value=0):
...
def play_dtmf_tone(key):
    for x in range(1000, 5000):
        tone = generate_dtmf_tone(key, duration, sample_rate, x)
        time.sleep(0.1)
    
    # Play the generated tone
    sd.play(tone, sample_rate)

# Example usage: Play the DTMF tone for key '5'
play_dtmf_tone('6')
#play_dtmf_tone('1')
#play_dtmf_tone('3')
#play_dtmf_tone('3')
